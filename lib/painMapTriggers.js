...
export function evaluatePainMap(painMap) {
  ...
  const l5 = stripe(zones, ['great toe', 'inner foot'], 'lateral shin') ||
             stripe(zones, ['great toe', 'inner foot'], 'lateral calf');
  if (l5) {
    fired.push({
  ...
  const s1 = stripe(zones, ['little toe', 'outer foot', 'heel'], 'calf');
  if (s1) {
    fired.push({
