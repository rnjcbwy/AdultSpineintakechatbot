...
/**
 * Styles a patient can give a symptom of their own.
 *
 * The six built-in types cover most spine complaints but not all of them —
 * itching, cramping, coldness, a pulling sensation. Rather than force those
 * into "aching", the patient can name up to four and pick a marker. Each style
 * differs in BOTH colour and shape, for the same reason the built-ins do: the
 * drawing has to survive a black-and-white printout and a colour-blind reader.
 */
export const CUSTOM_MARKER_STYLES = [
  { style: 'pink', shape: 'triangle', color: '#DB2777' },
  { style: 'green', shape: 'cross', color: '#65A30D' },
  { style: 'sky', shape: 'hexagon', color: '#0891B2' },
  { style: 'brown', shape: 'droplet', color: '#92400E' },
];

export const MAX_CUSTOM_TYPES = CUSTOM_MARKER_STYLES.length;

/** Custom types are stored on the map itself, so they travel with the drawing. */
export function customTypes(painMap) {
  return (painMap?.customTypes || []).filter((c) => c && c.id && c.label);
}

/** Built-in types plus whatever the patient defined. */
export function allTypes(painMap) {
  return [...SYMPTOM_TYPES, ...customTypes(painMap)];
}

/**
 * Resolve a marker type id.
 *
 * `types` is the list to search — pass allTypes(painMap) anywhere a custom
 * symptom might appear. Falling back to the first built-in keeps every caller
 * total: a mark whose custom type was deleted still renders as something.
 */
export function getType(id, types = SYMPTOM_TYPES) {
  return types.find((t) => t.id === id) || SYMPTOM_TYPES.find((t) => t.id === id) || SYMPTOM_TYPES[0];
}
...
export function summarizePainMap(painMap) {
  if (!painMap) return '';
  const lines = [];
  // Include the patient's own symptom names, or a custom mark would be
  // summarised as "aching" and the word they chose would be lost.
  ...
  for (const view of VIEWS) {
    ...
    for (const m of marks) {
      ...
      seen.add(key);
      parts.push(`${getType(m.type, types).label.toLowerCase()} at the ${m.zone}`);
    }
    for (const p of paths) {
      if (!p.fromZone || !p.toZone) continue;
      parts.push(`${getType(p.type, types).label.toLowerCase()} radiating from the ${p.fromZone} to the ${p.toZone}`);
    }
...
export const EMPTY_PAIN_MAP = {
  ...Object.fromEntries(VIEWS.map((v) => [v, { marks: [], paths: [] }])),
  customTypes: [],
};
