...
import { rosAnswer } from './rosSystems';
...
export function detectRedFlags(intakeData) {
  ...
  for (const flag of RED_FLAG_DEFINITIONS) {
    ...
    // Check ROS responses.
    //
    // These read through rosAnswer() rather than off the top of the object.
    // The answers have always lived one level down, under the body system that
    // asks them (reviewOfSystems.constitutional.unexplainedWeightLoss), so the
    // old direct reads resolved to undefined and these flags could only ever
    // fire from free text. Looking the answer up by question id also survives a
    // question moving between systems, which is exactly what happened when the
    // catch-all "Safety Screening" group was dissolved.
    if (!isTriggered && intakeData.reviewOfSystems) {
      ...
      for (const questionId of ROS_TRIGGERS[flag.id] || []) {
        if (rosAnswer(ros, questionId) === 'yes') {
          isTriggered = true;
          break;
        }
      }
