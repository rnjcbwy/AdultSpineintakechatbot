 ...
 */
async function buildCombinedPdf({ intake, readiness, docs }) {
  ...
  for (const cat of withDocs) {
    ...
    for (const doc of catDocs) {
      let blob;
      try { blob = await getDocumentBlob(doc.id); } catch { blob = null; }
 ...
 */
export async function buildPacket({ intake, readiness }) {
  const docs = intake?.documents || [];
  const zip = new JSZip();

  const { bytes, chapters, skipped } = await buildCombinedPdf({ intake, readiness, docs });
